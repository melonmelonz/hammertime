  ...
  return (
    <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link to="/builder">
          <Button variant="primary" icon={<PlusIcon />}>New Roster</Button>
        </Link>
      </div>
      ...
      {rosters.length === 0 ? (
        <motion.div
          ...
          className="text-center py-24 rounded-xl border-2 border-dashed border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/30"
        >
          ...
          <p className="text-neutral-400 text-sm mb-6">Build your first army to get started</p>
          <Link to="/builder">
          ...
          {rosters.map((roster, i) => (
            <motion.div
              ...
              className={cn(
                ...
                'hover:border-neutral-300 dark:hover:border-neutral-700 hover:shadow-sm transition-all duration-150',
              )}
            >
              <div className="px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <Badge variant="muted" size="sm">{totalUnits(roster)} units</Badge>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap text-xs text-neutral-400 mb-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link to="/builder" onClick={() => setActiveRoster(roster.id)}>
                    <Button variant="primary" size="sm" icon={<PencilSquareIcon />}>Edit</Button>
                  </Link>
                  <Button
                    variant="ghost"
                    size="sm"
                    icon={<TrashIcon />}
                    onClick={() => {
                      if (confirm(`Delete "${roster.name}"? This cannot be undone.`)) deleteRoster(roster.id)
                    }}
                    className="text-neutral-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
              ...
              {roster.forces.length > 0 && (
                <div className="border-t border-neutral-100 dark:border-neutral-800 px-5 py-2.5 bg-neutral-50 dark:bg-neutral-900/50 flex flex-wrap gap-4">
                  {roster.forces.map((force) => (
                    <div key={force.id} className="text-xs text-neutral-500">
                      <span className="text-red-600 dark:text-red-500 font-semibold">{force.catalogueName}</span>
